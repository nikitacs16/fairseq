...
def tokenize(sentence_array):
  #expected input: array of words in the sentence
  new_sentence_array = []
  for s in sentence_array:
    if '.' in s or '_' in s:
      temp = s.replace('.',' . ').replace('_',' _ ').split()
    else:
      temp = [s]
    new_sentence_array = new_sentence_array + temp

  return new_sentence_array

def convert_to_flat_db(db): #has the schema as string
  new_db = {}
  for k in db:
...
def get_updated_id(curr_table_vocab,train_table_vocab):
  ...
  for k in curr_table_vocab:
    if k not in common_keys:
      ...
      del train_table_vocab[uncommon_keys[c]]
      c = c + 1
...
def write_interaction(interaction_list,split,output_dir,flat_db,output_vocab):
  json_split = os.path.join(output_dir,split+'.json')
  pkl_split = os.path.join(output_dir,split+'.pkl')
  input_split = os.path.join(output_dir, split + '.input')
  output_split = os.path.join(output_dir, split + '.sql')
  input_vocab = defaultdict(int)
  table_vocab = defaultdict(int)
  ...
  for i, obj in enumerate(interaction_list):
    ...
    concat_s = flat_db[obj['database_id']] + ' ' + EOT + ' '
    if obj['database_id'] not in seen_ids:
      for s in concat_s.split():
        if s not in output_vocab:
          table_vocab[s]+=1

      seen_ids.append(table_vocab)
    
    new_interaction = []
    for ut in obj["interaction"]:
      sql = ut["sql"]
      utterance = ' '.join(k for k in tokenize(ut['utterance'].strip().lower().split()))
      ...
      for sql in sqls:
        ...
        if c >2:
          print(sqls)
        results = []
        tokenized_sql = sql.lower().split()
        tok_sql_list.append((tokenized_sql, results))
      ut["sql"] = tok_sql_list
      #sql_raw = ' '.join(k for k in tok_sql_list[0][0])
      #print(sql)
      corrected_tokens = tokenize(tok_sql_list[0][0])
      sql_raw = ' '.join(k for k in corrected_tokens)
      new_interaction.append(ut)
      concat_s = concat_s.strip() + ' ' + utterance
      pre_append = concat_s
...
def preprocess(dataset, remove_from=False):
  ...
  output_vocab = ['.','*','t1', 't2', '=', 'select', 'from', 'as', 'value', 'join', 'on', ')', '(', 'where', 't3', 'by', ',', 'count', 'group', 'order', 'distinct', 't4', 'and', 'limit', 'desc', '>', 'avg', 'having', 'max', 'in', '<', 'sum', 't5', 'intersect', 'not', 'min', 'except', 'or', 'asc', 'like', '!', 'union', 'between', 't6', '-', 't7', '+', '/','<EOV>']
  if remove_from:
    output_vocab = ['.','*','=', 'select', 'value', ')', '(', 'where', ',', 'count', 'group_by', 'order_by', 'distinct', 'and', 'limit_value', 'limit', 'desc', '>', 'avg', 'having', 'max', 'in', '<', 'sum', 'intersect', 'not', 'min', 'except', 'or', 'asc', 'like', '!=', 'union', 'between', '-', '+', '/', '<EOV>']
  ...
  flat_db = convert_to_flat_db(schema_tokens)
