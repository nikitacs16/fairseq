...
@register_model('sepseq2seq')
class SepSeq2Seq(FairseqEncoderDecoderModel):
...
class LSTMSepDecoder(FairseqIncrementalDecoder): #dictionary has to be target dictionary with the OOV included!
	"""LSTM decoder."""
	def __init__( 
		self, dictionary, sql_dictionary_size=52, embed_dim=512, hidden_size=512, out_embed_dim=512,
		num_layers=1, dropout_in=0.1, dropout_out=0.1, attention=True, 
		encoder_table_output_units=512, 
		encoder_source_output_units=512, 
		encoder_sql_output_units=512, 
		pretrained_embed=None, pretrained_embed_dim=None,
		...
		if attention:
			...
			self.table_attention = AttentionLayer(hidden_size, encoder_table_output_units, hidden_size, bias=False) #check this
			self.source_attention = AttentionLayer(hidden_size, encoder_source_output_units, hidden_size, bias=False) #check this
			self.sql_attention = AttentionLayer(hidden_size, encoder_sql_output_units, hidden_size, bias=False)#check this
			self.o_k = Linear(4*hidden_size,hidden_size)
	...
	def extract_features(
		...
		encoder_table_padding_mask = encoder_out['encoder_table_padding_mask']
		encoder_table_out = encoder_out['encoder_table_out']
		
		encoder_source_padding_mask = encoder_out['encoder_source_padding_mask']
		encoder_source_out = encoder_out['encoder_source_out']
		
		encoder_sql_padding_mask = encoder_out['encoder_sql_padding_mask']
		encoder_sql_out = encoder_out['encoder_sql_out']
		...
		attn_table_scores = x.new_zeros(srclen, seqlen, bsz) if self.attention is not None else None
		attn_source_scores = x.new_zeros(srclen, seqlen, bsz) if self.attention is not None else None
		attn_sql_scores = x.new_zeros(srclen, seqlen, bsz) if self.attention is not None else None
		...
		for j in range(seqlen):
			...
			out_table, attn_table_scores[:, j, :] = self.attention(hidden, encoder_table_outs, encoder_table_padding_mask)
			out_source, attn_source_scores[:, j, :] = self.attention(hidden, encoder_table_outs, encoder_source_padding_mask)
			out_sql, attn_source_scores[:, j, :] = self.attention(hidden, encoder_sql_outs, encoder_sql_padding_mask)
			out_cat = torch.cat((out_table, out_source, out_sql, hidden),1)
			out =  F.tanh(self.o_k(out_cat))		
		...
		if not self.training and self.need_attn and self.attention is not None:
			attn_table_scores = attn_table_scores.transpose(0, 2)
			attn_source_scores = attn_source_scores.transpose(0, 2)
			attn_sql_scores = attn_sql_scores.transpose(0, 2)


		else:
			attn_scores = None
			attn_copy_scores = None
			attn_sql_scores = None
		
		return x, attn_table_scores, attn_source_scores, attn_sql_scores
